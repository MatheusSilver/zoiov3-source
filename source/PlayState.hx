...
class PlayState extends MusicBeatState
{
	...
	private var camFollowPos:FlxObject; //USAR ISSO AQUI EM MUSICAS ESPECIFICAS
	private static var prevCamFollow:FlxPoint;
	private static var prevCamFollowPos:FlxObject;
	private static var resetSpriteCache:Bool = false;

	//Vai tomar no seu cu filha da puta do caralho odeio code desgraça
	var songinfo:FlxSprite; //OTIMIZAR DEPOIS
	...
	public var camHUD2:FlxCamera; // jumpscares, ect.. //Eu peguei do Sonic EXE
	...
	//CICLOS
	var Jumpuscara:FlxSprite = new FlxSprite(0, 0);
	...
	//ZAPURGATION
	var tutorialTextoFoda:FlxText; 
	var particulas:FlxSprite; 
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'LordXStage': // Isso é meio sus pq eu tive que incrementar hard code e soft code
			{
				defaultCamZoom = 0.8;
				curStage = 'LordXStage';

				var sky:FlxSprite = new FlxSprite(-1900, -1006).loadGraphic(Paths.image('lordx/sky'));
				...
				var hills1:FlxSprite = new FlxSprite(-1440, -806 + 200).loadGraphic(Paths.image('lordx/hills1'));
				...
				var floor:FlxSprite = new FlxSprite(-1400, -496).loadGraphic(Paths.image('lordx/floor'));
				...
				eyeflower.frames = Paths.getSparrowAtlas('LordXStage/WeirdAssFlower_Assets', 'zoio');
				...
				hands.frames = Paths.getSparrowAtlas('LordXStage/NotKnuckles_Assets', 'zoio');
				...
				var smallflower:FlxSprite = new FlxSprite(-1500, -506).loadGraphic(Paths.image('lordx/smallflower'));
				smallflower.setGraphicSize(Std.int(smallflower.width * .6));
				smallflower.antialiasing = true;
				smallflower.scrollFactor.set(1.5, 1);
				smallflower.active = true;
				add(smallflower);

				var bFsmallflower:FlxSprite = new FlxSprite(-1500 + 300, -506 - 50).loadGraphic(Paths.image('lordx/smallflower'));
				...
				bFsmallflower.active = true;
				bFsmallflower.flipX = true;
				add(bFsmallflower);

				var smallflowe2:FlxSprite = new FlxSprite(-1500, -506).loadGraphic(Paths.image('lordx/smallflowe2'));
				smallflowe2.setGraphicSize(Std.int(smallflower.width * .6));
				smallflowe2.antialiasing = true;
				smallflowe2.scrollFactor.set(1.5, 1);
				smallflowe2.active = true;
				add(smallflowe2);

				var tree:FlxSprite = new FlxSprite(-1900 + 650 - 100, -1006 + 350).loadGraphic(Paths.image('lordx/tree'));
			...
			case 'stage': //confronting yourself muito fodas 
	...
	//Bom, praticamente isso por enquanto ta aqui pra nada, mas vai que e usado ne?
	//Whatever, vou usar sim
	//OTIMIZAR DEPOIS
	function jump() //Roubei do Sonic Exe e obrigado Matheus Silver
		{
			if (!ClientPrefs.jumpscare) {
			trace('JUMPSCARE aaaa');
			Jumpuscara.frames = Paths.getSparrowAtlas('eversonZOIO', 'zoio');
			Jumpuscara.animation.addByPrefix('jump', 'zoioSPOOK', 24, false);
			Jumpuscara.screenCenter();
			Jumpuscara.scale.x = 1.1;
			Jumpuscara.scale.y = 1.1;
			Jumpuscara.y += 370;
			Jumpuscara.cameras = [camHUD2];
			FlxG.sound.play(Paths.sound('goofy', 'zoio'), 1);
			add(Jumpuscara);
			Jumpuscara.animation.play('jump');
			Jumpuscara.animation.finishCallback = function(pog:String)
			{
				trace('ended jump');
				remove(Jumpuscara);
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingString == '?') {
			scoreTxt.text = 'Pontuacao: ' + songScore + ' | Erros: ' + songMisses + ' | Nota: ' + ratingString;
		} else {
			scoreTxt.text = 'Pontuacao: ' + songScore + ' | Erros: ' + songMisses + ' | Nota: ' + ratingString + ' (' + Math.floor(ratingPercent * 100) + '%)';
		}
		...
		if (FlxG.keys.justPressed.ENTER && startedCountdown && canPause)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				if (FlxG.random.bool(2.5)) //lil hack XDDDD
				{
	...
	override function stepHit()
	{
		...
		//Musicas e seus sprites legais (PEGUEI DO ARROW FUNK)
		if (curStep == 1)
			{
		...
		if (curSong == 'Zapurgation')
			{
				if (curStep == 1) //Vou fazer uma pequena coisa, que e pro sistema de menus
					{
						ClientPrefs.zapurgacao = true;
						ClientPrefs.naovenceu = false;
					}

				if (curStep == 2) //Caso você seja burro esse negócio tá aqui
					{
						tutorialTextoFoda = new FlxText(0, 0, FlxG.width,
							"Dica: Não aperte a nota Hitkill.\n
							(ou aperte e veja o que acontece)",
							32);
							tutorialTextoFoda.setFormat("VCR OSD Mono", 32, FlxColor.WHITE, CENTER);
							tutorialTextoFoda.screenCenter(Y);
						add(tutorialTextoFoda);
					}
				
				if (curStep == 64)
					{
						tutorialTextoFoda.visible = false;
					}
			...
			//if (curSong == 'ConfrontingYourself') // essa merda basicamente não funciona
		...
		for (i in 0...arrayIDs.length) {
			if(!Achievements.achievementsUnlocked[arrayIDs[i]][1]) {
				switch(arrayIDs[i]) {
					...
					case 16: //burrice be like
					if(Paths.formatToSongPath(SONG.song) == 'zapurgation' && !usedPractice) {
						Achievements.unlockAchievement(arrayIDs[i]);
						ClientPrefs.zapurgacao = true;
						ClientPrefs.naovenceu = false;
						return arrayIDs[i];
